<template>
    <div>
        ...
        <div v-if="view_detalles">
            ...
            <div v-if="all_detalles.donaciones.legth > 0">
                <b-row>
...
import LoadComponent from '../cortes/partials/LoadComponent.vue';
    export default {
        ...
        methods: {
            rowClass(item, type) {
                var entradas = parseInt(item.entradas) + parseInt(item.devoluciones);
                var salidas = parseInt(item.remisiones) + parseInt(item.notas) + parseInt(item.promociones) + parseInt(item.donaciones) + parseInt(item.entdevoluciones);
